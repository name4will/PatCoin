Patcoin Core version *0.15.1* is now available from:

  <https://download.patcoin.org/patcoin-0.15.1/>
...
A number of changes to the way Patcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 mehrobs per kB (0.00001 PAT). This is to prepare for dropping the minimum transaction fee to 0.1 mehrobs per kB (0.0001 PAT) in the next release.
